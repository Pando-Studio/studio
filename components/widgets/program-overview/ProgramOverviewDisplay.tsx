...
export function ProgramOverviewDisplay({ data }: WidgetDisplayProps) {
  const d = data as unknown as ProgramOverviewData;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-lg font-semibold">{d.title || 'Programme'}</h3>
      </div>

      <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
        {d.credits && (
          <span className="flex items-center gap-1">
            <Award className="h-3 w-3" /> {d.credits} ECTS
          </span>
        )}
        {d.duration && (
          <span className="flex items-center gap-1">
        ...
        {d.level && (
          <span className="bg-primary/10 text-primary px-2 py-0.5 rounded">{d.level}</span>
        )}
      </div>
